...
function Queue(name, opts) {
  ...
  this.name = name;
  this.tasks = [];
  this._running = []; // weakmap here sure would be nice
  this.workers = 0;
  this.paused = false;
...
Queue.prototype = {
  push: function(name, isUnique, fn, meta) {
    ...
    var isDup = this.tasks.concat(this._running).some(function(t) {
  ...
  _run: function() {
    ...
    var done = function _done(task) {
      return function __done() {
        var args = Array.prototype.slice.call(arguments);
        self.workers -= 1;

        self._removeRunning(task);
        self.emit.apply(self, [EVENTS.DONE].concat(args, self.name, task.name, task.meta));
    ...
    this._running = this._running.concat(tasks);
    tasks.forEach(function(t) {
      t.fn.call(t.fn, once(done(t)));
      this.emit(EVENTS.RUN, this.name, t.name, t.meta);
    }, this);
  },

  _removeRunning: function(t) {
    var idx = indexOf(this._running, function(task) {
      return task.id === t.id;
    });
...
extend(MultiQueue.prototype, {
  ...
  push: function(key, task, opts) {
    ...
    if (is.not.fn(task)) {
      throw new TypeError('Epected `task` to be a function. Got ' + typeof task);
    }
